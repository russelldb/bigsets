...
clock_key(Set) ->
    ...
    sext:encode({s, Set, clock, 0, <<>>, 0, <<0:1>>}).
...
epoch_rollup_key(Set, Epoch) ->
    sext:encode({s, Set, epoch, Epoch, <<>>, 0, <<0:1>>}).
...
-spec insert_member_key(set(), epoch(), member(), actor(), counter()) -> key().
insert_member_key(Set, Epoch, Elem, Actor, Cnt) ->
    sext:encode({s, Set, Epoch, Elem, Actor, Cnt, <<0:1>>}).

-spec remove_member_key(set(), epoch(), member(), actor(), counter()) -> key().
remove_member_key(Set, Epoch, Element, Actor, Cnt) ->
    sext:encode({s, Set, Epoch, Element, Actor, Cnt, <<1:1>>}).
