...
: ${RTEE_DEST_DIR:="$HOME/rt/bigset"}

cd "$( dirname "${BASH_SOURCE[0]}" )/../../"
BIGSET_DIR=$(pwd)

echo "Making $BIGSET_DIR the current release:"
...
#sed -i -e "s#TESTS_PATH#$BIGSET_DIR\/riak_test#g" -e "s#RTDEV_PATH#$RTEE_DEST_DIR#g" $HOME/.riak_test.config
...
cp -p -P -R $BIGSET_DIR/dev current
