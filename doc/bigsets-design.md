...
screaming "Deltas! Deltas!" right now.) However, even if we optimize
...
Below is a basho_bench run adding one hundred thousand random keys to
...
keys. At least one key per element, and an extra key for meta data.
...
* Do you contain 'X'?
* How many elements start with "Ru's*"?
